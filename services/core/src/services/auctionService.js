// services/core/src/services/auctionService.js
...
async function createAuction({ payload, correlationId }) {
  ...
  try {
    ...
    // Extraemos la latitud y longitud del local desde el payload
    const shopLat = payload.cancellation.slot.latitud || null;
    const shopLon = payload.cancellation.slot.longitud || null;

    // Pasamos las coordenadas a la función de ranking
    const rankedCandidates = rankCandidates(payload.waitlist, shopLat, shopLon, DEFAULT_WEIGHTS);
